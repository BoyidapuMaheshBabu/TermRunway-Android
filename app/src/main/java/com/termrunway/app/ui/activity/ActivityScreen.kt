...
fun ActivityScreen(
    ...
    Column(modifier.fillMaxSize()) {
        ...
        Row(Modifier.fillMaxWidth().padding(horizontal = 12.dp), verticalAlignment = Alignment.CenterVertically) {
            IconButton(onClick = { selectedDate = addDays(selectedDate, -1) }) { Icon(Icons.Outlined.ArrowBack, "Previous day") }
            ...
            IconButton(enabled = selectedDate < today, onClick = { selectedDate = addDays(selectedDate, 1) }) { Icon(Icons.Outlined.ArrowForward, "Next day") }
            IconButton(onClick = {
                val initial = Calendar.getInstance().apply { timeInMillis = selectedDate }
                DatePickerDialog(
                    androidx.compose.ui.platform.LocalContext.current,
                    { _, year, month, day ->
                        selectedDate = startOfDay(Calendar.getInstance().apply {
        ...
        if (dayExpenses.isEmpty() && dayIncome.isEmpty()) {
            Column(Modifier.fillMaxWidth().weight(1f).padding(32.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
            ...
            LazyColumn(
                ...
                if (dayIncome.isNotEmpty()) {
                    ...
                    items(dayIncome, key = { "income-" + it.id }) { item ->
                        TransactionRow(item.source, item.amountCents, true, item.dateMillis, item.note) { onEdit(TransactionEditRequest(income = item)) }
                    }
                }
                if (dayExpenses.isNotEmpty()) {
                    ...
                    items(dayExpenses, key = { "expense-" + it.id }) { item ->
                        TransactionRow(item.category, item.amountCents, false, item.dateMillis, item.note) { onEdit(TransactionEditRequest(expense = item)) }
                    }
                }
            }
        }
...
private fun SummaryCard(label: String, value: String, modifier: Modifier) {
    Card(modifier = modifier, shape = RoundedCornerShape(18.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceVariant)) {
...
@Composable
private fun TransactionRow(title: String, amount: Long, income: Boolean, timeMillis: Long, note: String, onClick: () -> Unit) {
    Card(onClick = onClick, modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(18.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface)) {
        Row(Modifier.fillMaxWidth().padding(14.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            Column(Modifier.weight(1f)) {
                ...
                Text(formatTime(timeMillis) + if (note.isBlank()) "" else " · " + note, style = MaterialTheme.typography.labelMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
